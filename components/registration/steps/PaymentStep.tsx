...
export const PaymentStep = ({ data, onBack, onSuccess }: PaymentStepProps) => {
  ...
  useEffect(() => {
    const checkPaymentCallback = async () => {
      ...
      if (paymentRef && !paymentCompleted) {
        ...
        try {
          const verificationResponse = await PAYMENT_API.VERIFY_PAYMENT(paymentRef);
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      try {
        const progressResponse = await REGISTRATION_API.GET_PROGRESS(userData.id);
      ...
      if (registrationData?.billingFrequency !== data.billingFrequency) {
        toast.error("Billing frequency mismatch. Please go back and reselect your billing frequency.");
        setIsProcessing(false);
        return;
      }
      ...
      if (response.error) {
        ...
        if (errorMessage.includes("Membership type mismatch")) {
          ...
          errorMessage = "Billing frequency doesn't match your registration. Please go back and reselect your billing frequency.";
        } else if (errorMessage.includes("Invalid payment amount")) {
          errorMessage = "Payment amount doesn't match the selected plan. Please go back and reselect your membership.";
        }
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="bg-green-50 border border-green-200 rounded-lg p-6">
          <h3 className="font-semibold text-green-900 mb-4">Membership Activated</h3>
          <div className="space-y-3">
            <div className="flex justify-between">
              <span className="text-green-700">Plan</span>
              <span className="font-medium text-green-900">{data.membershipType.replace('_', ' ').toUpperCase()}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-green-700">Billing</span>
              <span className="font-medium text-green-900">{data.billingFrequency === "monthly" ? "Monthly" : "Annual"}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-green-700">Amount Paid</span>
              <span className="font-bold text-green-900">R {data.amount.toFixed(2)}</span>
            </div>
          </div>
        </div>
      ...
      <div className="bg-gray-50 rounded-lg p-6">
        ...
        <div className="space-y-3">
          <div className="flex justify-between">
            ...
            <span className="font-medium">{data.membershipType.replace('_', ' ').toUpperCase()}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-600">Billing</span>
            <span className="font-medium">{data.billingFrequency === "monthly" ? "Monthly" : "Annual"}</span>
          </div>
          <div className="flex justify-between">
      ...
      <div className="bg-gray-50 rounded-lg p-6">
        ...
        <div className="space-y-3">
          <div className={`flex items-center space-x-3 border rounded-lg p-4 ${
            paymentMethod === "paystack" ? "border-[#9FC93B] bg-green-50" : "border-gray-300"
          }`}>
        ...
        <div className="flex items-start space-x-2">
          ...
          <div>
            <p className="text-sm text-blue-800">
              ...
              you'll receive a confirmation email. You can manage your subscription from your dashboard.
            </p>
          </div>
        </div>
      </div>
